...
1. Set up and run the client application
    ...
    1. `npx takeshape init` - Follow the command prompts to set up your local environment to communicate with TS
...
- `static` - (configurable in takeshape.yml) Anything inside the static folder will be copied over into the build folder. If you have a build process to generate files like scss into css or combine js files they should output into the static folder or in subdirectories of the static folder.
...
- `takeshape.yml` - Configuration options to build your project including paths to relevant directories, routes and data contexts

#### `takeshape.yml` Configuration Options
...
- `route(routeName: String)`
  ...
  {{ post | route('posts') }}
  ```
  Returns a relative path to a piece of content as defined by the routes in `takeshape.yml`. The input Object must have the necessary fields specified in the route path in order to construct the path properly.
- `md`
  ```
  {{ markdown | md }}
  ```
...
- `date(format: String|Object)`
  ...
  ```
  Formats dates using [moment.js](https://momentjs.com/). `format` can be either a [format string](https://momentjs.com/docs/#/displaying/format/) or an object where you can specify a format and override the default timezone and locale (configured in `takeshape.yml`).
